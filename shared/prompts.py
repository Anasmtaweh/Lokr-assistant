...
- **EXPRESS MIDDLEWARE OR-LOGIC RULE**: Never chain role-checking middlewares (like `userMiddleware` and `adminMiddleware`) sequentially in a route definition if you want an OR condition (e.g., "admin OR owner"). Sequential middlewares execute as an AND condition. Instead, use only the base authentication middleware (e.g., `userMiddleware`) to set `req.user`, and perform the OR logic explicitly inside the route handler block.
